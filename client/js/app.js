...
let state = {
  ...
  activeBatchId: null,
  eventSource: null,
  username: localStorage.getItem('petrorag_username') || null
};
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Resume batch monitoring if one was active
  const savedBatchId = localStorage.getItem('petrorag_active_batch_id');
  if (savedBatchId) {
      console.log(`[Batch] Resuming monitoring for batch: ${savedBatchId}`);
      startBatchMonitoring(savedBatchId);
  }
});
...
function updateUsernameUI() {
  ...
  if (display) display.innerText = state.username || 'Guest';
}
...
function saveWelcomeUsername() {
  ...
  if (!newUsername) {
      showToast('Please enter a username to continue', 'warning');
      return;
  }
  
  state.username = newUsername;
  localStorage.setItem('petrorag_username', newUsername);
  showToast(`Welcome, ${newUsername}!`, 'success');
  updateUsernameUI();
  closeModal('welcome-modal');
  refreshAll(); // Load data now that we have a user
}
...
function saveProfileUsername() {
  ...
  if (newUsername !== state.username) {
      state.username = newUsername;
      localStorage.setItem('petrorag_username', newUsername);
      showToast(`Profile updated to ${newUsername}`, 'success');
      updateUsernameUI();
      refreshAll(); // Reload user-specific data
  }
...
function startBatchMonitoring(batchId) {
  // Persist active batch immediately
...
function stopBatchMonitoring() {
  ...
  localStorage.removeItem('petrorag_active_batch_id');
  loadDocuments(state.selectedCategoryId);
...
async function loadChats() {
  try {
      state.chats = await Api.listChats(state.username);
      renderSidebarChats();
  } catch (e) {
...
function renderMessageContent(msg) {
  ...
  if (msg.role === 'user') {
      html += `<div class="message-header"><i class="fa-solid fa-user-astronaut"></i> ${state.username || 'You'}</div>`;
  }
...
async function deleteChat(id) {
  ...
  try {
      await Api.deleteChat(id, state.username);
      state.selectedChatId = null;
      await loadChats();
      switchView('dashboard');
...
window.saveWelcomeUsername = saveWelcomeUsername;
window.saveProfileUsername = saveProfileUsername;
